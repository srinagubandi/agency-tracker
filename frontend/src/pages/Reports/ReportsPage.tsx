...
const ReportsPage: React.FC = () => {
  ...
  return (
    <div>
      ...
      <Tabs>
        <Tab active={tab==='employee'} onClick={()=>setTab('employee')}>By Employee</Tab>
        <Tab active={tab==='client'} onClick={()=>setTab('client')}>By Client</Tab>
        <Tab active={tab==='campaign'} onClick={()=>setTab('campaign')}>By Campaign</Tab>
      </Tabs>
      <FilterBar>
        ...
        {tab!=='client'&&<StyledSelect value={selectedClient} onChange={e=>setSelectedClient(e.target.value)}><option value="">All Clients</option>{clients.map((c:any)=><option key={c.id} value={c.id}>{c.name}</option>)}</StyledSelect>}
        <Button variant="primary" size="small" startIcon={<Icon name="filter" size={14}/>} onClick={fetchReport}>Apply</Button>
      </FilterBar>
      <Card>
        <Table>
          <TableHead><TableRow>
            {tab==='employee'&&<><TableCellHeader>Employee</TableCellHeader><TableCellHeader>Role</TableCellHeader></>}
            {tab==='client'&&<TableCellHeader>Client</TableCellHeader>}
            {tab==='campaign'&&<><TableCellHeader>Campaign</TableCellHeader><TableCellHeader>Client</TableCellHeader></>}
          ...
          <TableBody>
            ...
            {data.map((row:any,i:number)=>(
              <TableRow key={i}>
                {tab==='employee'&&<><TableCell style={{fontWeight:500}}>{row.name||row.user_name}</TableCell><TableCell style={{fontSize:12,color:'#6b7280'}}>{row.role}</TableCell></>}
                {tab==='client'&&<TableCell style={{fontWeight:500}}>{row.name||row.client_name}</TableCell>}
                {tab==='campaign'&&<><TableCell style={{fontWeight:500}}>{row.name||row.campaign_name}</TableCell><TableCell style={{fontSize:13}}>{row.client_name}</TableCell></>}
