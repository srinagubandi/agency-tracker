...
const Tab = styled('button', { shouldForwardProp: (prop) => prop !== 'active' })<{active:boolean}>`padding:8px 20px;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;background:${({active})=>active?'#fff':'transparent'};color:${({active})=>active?'#2E6DA4':'#6b7280'};box-shadow:${({active})=>active?'0 1px 4px rgba(0,0,0,0.1)':'none'};`;
