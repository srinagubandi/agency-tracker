...
const NotifItem = styled('div', { shouldForwardProp: (prop) => prop !== 'unread' })<{unread:boolean}>`display:flex;align-items:flex-start;gap:14px;padding:16px 20px;border-bottom:1px solid #f3f4f6;background:${({unread})=>unread?'#f0f7ff':'#fff'};&:last-child{border-bottom:none;}`;
const NotifDot = styled('div', { shouldForwardProp: (prop) => prop !== 'unread' })<{unread:boolean}>`width:8px;height:8px;border-radius:50%;background:${({unread})=>unread?'#2E6DA4':'#d1d5db'};margin-top:6px;flex-shrink:0;`;
