...
const RoleBadge = styled('span', { shouldForwardProp: (prop) => prop !== 'role' })<{role:string}>`font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;background:${({role})=>role==='super_admin'?'#dbeafe':role==='manager'?'#e0f2fe':role==='worker'?'#dcfce7':'#f3f4f6'};color:${({role})=>role==='super_admin'?'#2563eb':role==='manager'?'#0284c7':role==='worker'?'#16a34a':'#6b7280'};`;
